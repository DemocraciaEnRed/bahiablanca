  ...
  User.findByEmail(email, function (err, user){
    ...
    api.token.createPasswordResetToken(user, meta, function (err, token) {
      ...
      if (config.smsValidation) {
        log('Sending SMS for forgot-password for account: %s', user.email)

        var message = 'Solicitaste recuperar tu clave de Demos Bahia Blanca. Para continuar, ingresa el siguiente codigo: ' + token.id;

        if (!user.phone) {
          var error = new Error('El usuario no tiene un número de teléfono asociado.');
          return callback(err);
        }

        sms.send(user.phone, message, callback);
      } else if (notifier.enabled()) {
        log('Sending email for forgot-password for account: %s', user.email)

        var resetUrl = utils.buildUrl(config, {pathname: '/forgot/reset/' + token.id});

        notifier.notify('forgot-password')
          .to(user.email)
          .withData({resetUrl: resetUrl})
        ...
        log('Notifier is disabled: unable to send pasword-reset mail to user');
        return callback();
      }
    });
  });
