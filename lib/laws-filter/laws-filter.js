...
LawsFilter.prototype.getParent = function(id) {
  ...
  var match = this.items().filter(function(item) {
    return item.id === id || any(item.children, function(child) { return child.id === id; });
  });
...
LawsFilter.prototype.reload = function() {
  ...
  items = items
    .filter(function(law) { return !law.parent })
    .map(function(law) {
      law.children = laws.getChildren(law.id);
      return law;
    });
...
LawsFilter.prototype.middleware = function(ctx, next) {
  this.ready(next);
};
