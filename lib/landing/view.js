...
function LandingView () {
  ...
  .on('change', function(value) {
    if ('0' === value.id) {
      view.list.sort(value.id, { sortFunction: randomSort });
    } else {
...
LandingView.prototype.switchOn = function() {
  this.list = new List('landing', { valueNames: ['law-title', 'law-closing-date', 'law-author', 'law-tag', 'law-keywords'] });
