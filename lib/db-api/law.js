...
exports.all = function all(fn) {
  ...
  var fields = ['id lawId mediaTitle tag participants votes createdAt updatedAt',
    'closingAt publishedAt deletedAt status open closed links author authorUrl',
    'authorPictureUrl parent order summary styles imageUrl keywords party partyUrl twitterHandler'];
...
exports.create = function create(data, fn) {
  ...
  ensureTag(data, function(err, data) {
    if (err) return log('Found error from law creation: %s', err.message), fn(err);
    createLaw(data, fn);
  });
...
function ensureTag(data, fn) {
  tagApi.search(data.hash, function(err, tags) {
...
/**
 * Update a law and its children with `data`
 */

exports.updateChildren = function updateChildren(id, data, fn) {
  Law.update({ $or: [{ parent: id }, { _id: id }]}, data, { multi: true },
    function (err, count) {
      if (err) return fn(err);
      log('Updated %d documents', count);
...
/**
 * Get Law's participants, summing the participants of all its children. Only for parent laws.
 *
 * @param {String|ObjectId} id Law's `id`
 * @param {Function} fn callback function
 *   - 'err' error found while process or `null`
...
exports.getTotalParticipants = function getTotalParticipants(id, fn) {
  var query = { _id: id, deletedAt: null };
  ...
  .exec(function (err, law) {
    ...
    if (law.parent) {
      return fn(new Error('Law is not parent'));
    }

    Law.find({ parent: id }, function (err, laws) {
      if (err) return fn(err);
      var participants = laws
        .map(function (l) { return l.participants; })
        .reduce(function (prev, current) { return prev.concat(current); })
        .map(function (p) { return p.toString(); });

      return fn(null, unique(participants));
    });
  });
};
...
exports.vote = function vote(id, user, value, fn) {
  ...
  log('Proceding to vote %s at law %s by user %s', value, id, user.id || user);
  Law
  .findOne(query)
  .exec(function (err, law) {
    if (err) {
    ...
    law.vote(user.id, value, function(err) {
      if (err) {
        log('Found error %s', err);
        return fn(err);
      };

      log('Voted %s at law %s by user %s', value, id, user.id || user);
      fn(null, law);
    });
  });
};
...
exports.userComments = function userComments(id, userId, fn) {
  log('Get comments for law %s from user %s', id, userId);
