...
exports.search = function search(query, fn) {
  ...
  .select('firstName lastName fullName email avatar notifications')
  .exec(function(err, users) {
    if (err) {
      log('Found error: %j', err);
      return fn(err);
...
exports.getByEmail = function search(email, fn) {
  ...
  .select('firstName lastName fullName email avatar notifications emailValidated')
  .exec(function(err, user) {
    if (err) {
...
/**
 * Find `User` object by phone
 *
 * @param {String} The phone of the user
 * @param {Function} fn callback function
 *   - 'err' error found while process or `null`
 *   - 'user' the `User` object found or `undefined`
 * @return {Module} `user` module
...
exports.getByPhone = function search(phone, fn) {
  log('Searching for User with phone %s', phone);

  User.findOne({phone : phone})
  .select('firstName lastName fullName email phone avatar notifications emailValidated')
  .exec(function(err, user) {
    if (err) {
      return log('Found error: %j', err), fn(err);
    }
    ...
    if (!user) {
      return log('User not found for phone %s', phone), fn(null, false);
    };

    log('Found User %j for phone %s', user.id, phone)
    fn(null, user);
  });
 ...
 * e.g.: '/api/citizen/me' call from an authenticated user.
 *
 * @param {User} user.
...
exports.expose.confidential = function(user){
  return expose('id firstName lastName email avatar staff official notifications locale incomplete')(user);
}
...
exports.expose.ordinary = function(user){
  return expose('id fullName avatar notifications locale')(user);
}
