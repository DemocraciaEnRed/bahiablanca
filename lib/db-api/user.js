...
exports.search = function search(query, fn) {
  ...
  .select('firstName lastName fullName email phone dni avatar notifications')
  .exec(function(err, users) {
    if (err) {
      log('Found error: %j', err);
      return fn(err);
...
exports.getByEmail = function search(email, fn) {
  ...
  .select('firstName lastName fullName email phone dni avatar notifications emailValidated')
  .exec(function(err, user) {
    if (err) {
...
exports.expose.confidential = function(user){
  return expose('id firstName lastName email phone dni avatar staff official notifications locale incomplete')(user);
}
