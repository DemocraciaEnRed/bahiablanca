...
var ValidateCodeForm = require('./validation-code-form-view');
var dom = require('dom');
...
page('/signup/validated', externalSignup, citizen.required, parse, function(ctx, next) {
  ...
  if (!config.smsValidation && ctx.query.reference) {
    setTimeout(function () {
...
page('/signup/resend-validation-email', externalSignup, function(ctx, next) {
  ...
  // Update page's title
  title(t('signup.resend-validation-' + (config.smsValidation ? 'sms' : 'email')));

  // Empty container and render form
  form.replace('#content');
});

page('/signup/validate-code', parse, function (ctx) {
  var opts = {
    action: ctx.query.action
  };
  var form = new ValidateCodeForm(opts);
  classes(document.body).add('validation-code');
  form.replace('#content');
});
