...
/**
 * Basic official restriction middleware blocking
 * non-officla citizens to access some resources
 *
 * @param {Request} req
 * @param {Response} res
 * @param {Function} next
...
exports.official = function official(req, res, next) {
  if (!req.user) return log('Citizen not logged in'), res.send(403);
  if (!req.user.official) return log('Citizen is not from staff'), res.send(403);
  next();
}
