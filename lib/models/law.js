...
var LawSchema = new Schema({
  ...
  , links: [LinkSchema]
  // If it's null, its the parent
  , parent: { type: ObjectId, ref: 'Law' }
  // For children
...
/**
 * Calculates if author has twitter
 * based on their authorUrl
 *
 * @return {boolean} authorHasTwitter
 * @api public
 */

LawSchema.virtual('authorHasTwitter').get(function() {
  return this.authorUrl && !!~this.authorUrl.indexOf('twitter.com')
});
...
LawSchema.virtual('twitterHandler').get(function() {
  if (this.authorHasTwitter) {
    return this.authorUrl.split('twitter.com/')[1];
  } else {
    return '';
  }
});
